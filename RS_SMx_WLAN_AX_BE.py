...
class TXMode(Enum):
    """Valid transmission modes for SMW."""
    HE_SU = "HE_SU"    # 802.11ax Single-User
    HE_MU = "HE_MU"    # 802.11ax Multi-User
    EHT_SU = "EHT_SU"  # 802.11be Single-User MIMO
    EHT_MU = "EHT_MU"  # 802.11be Multi-User MIMO/OFDMA
    EHT_TB = "EHT_TB"  # 802.11be Trigger-Based PPDU
...
class RS_SMX:
    ...
    def create_waveform(self, standard: str, bw: str, mcs: str, burst_length: float, duty_cycle: float):
        """Creates and saves an 802.11 waveform (WAX or WBE) on the SMW."""
        try:
            # Validate duty cycle
            ...
            # Set standard and TX mode
            if standard == "WBE":
                self.opc_check("SOURce1:BB:WLNN:FBLOck1:STANdard WBE")
                tx_mode = f"EHT{bw_value[2:]}"  # e.g., EHT320
                header_duration = 43.2e-6  # EHT preamble duration (seconds)
            elif standard == "WAX":
                self.opc_check("SOURce1:BB:WLNN:FBLOck1:STANdard WAX")
                tx_mode = f"HE{bw_value[2:]}"  # e.g., HE80
                header_duration = 40e-6  # HE SU preamble duration (seconds)
            else:
                raise ValueError("Only 'WBE' (802.11be) and 'WAX' (802.11ax) are supported")
            self.opc_check(f"SOURce1:BB:WLNN:FBLOck1:TMOde {tx_mode}")

            # Set MCS
            mcs_index = int(mcs.replace("MCS", "", 1))  # Convert 'MCS13' to 13
            if standard == "WBE" and mcs_index not in self.ieee_80211be.MCS_DATA:
                raise ValueError(f"MCS index {mcs_index} is not valid for 802.11be")
            elif standard == "WAX" and mcs_index > 11:  # 802.11ax max MCS is 11
                raise ValueError(f"MCS index {mcs_index} exceeds 802.11ax maximum (11)")
            ...
            symbol_duration = 12.8e-6 + gi_value  # OFDM symbol duration + GI (same for AX and BE)
            ...
            self.write_command(':SOURce1:BB:WLNN:TRIGger:OUTPut1:MODE REST')
    ...
    def enable_output(self):
        ...
        self.write_command("OUTPut1:STATe 1")
        self.logger.info("RF output enabled.")

    def disable_output(self):
        """Disables the RF output."""
        self.write_command("OUTPut1:STATe 0")
        self.logger.info("RF output disabled.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        '''
        # 802.11ax waveform parameters (example)
        standard = "WAX"
        bw = "BW80"
        mcs = "MCS11"
        burst_length = 4e-3  # seconds
